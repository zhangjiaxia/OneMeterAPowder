<template>
	<view>
		<view class="uni-flex uni-row vertical addresslist" v-for="(item,index) in addressList" :key="index">
			...
			<view class="uni-flex right address" @click="$turnPage('/pages/center/address-add?item='+JSON.stringify(item), 'navigateTo')">编辑</view>
		</view>
		<view class="uni-flex content">
			<view class="uni-flex content addbtn" @click="$turnPage('/pages/center/address-add', 'navigateTo')">新建地址</view>
		</view>
	</view>
</template>
