	...
	<view>
		<view class="container">
			...
			<view class="detail-bottom">
				...
				<view class="shop-btn" @click="confrimOrderPage(true)">加入购物车</view>
				<view class="pay-btn" @click="confrimOrderPage(false)">立即购买</view>
		...
		<view class="confirm-modal" v-if="confirmModal" @touchmove.stop.prevent="touch">
			<view class="confirm-modal-content">
				<view class="modal-shop-message">
					...
					<image src="../../static/close.png" class="modal-shop-close" @click="confirmModal=false;"></image>
				</view>
				...
				<view class="modal-bottom" @click="handleConfirm">
					...
					<text class="modal-bottom-left" @click="handleConfirm">确认支付</text> -->
	...
	export default {
		...
		methods: {
			...
			addCart(code) {
				...
				interfaceurl.checkAuth(interfaceurl.cartCreate, params, false).then((res) => {
					uni.showToast({
						title: '购物车添加成功',
						icon: 'success',
						duration: 2000
...
<style>
	...
	.mask {
		...
		width: 100%;
		height: 100vh;
		/* fixed会滚动穿透,absolute并且配置catchtouchmove可以避免这个问题，但是高度要适应一下 */
		position: absolute;
		z-index: 1;
		opacity: 0.9;
		top: 0;
